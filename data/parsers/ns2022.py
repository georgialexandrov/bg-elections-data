#!/usr/bin/env python3
"""
ns2022.py — Parser for the October 2022 parliamentary elections.

One election:
  ns2022 — Народно събрание 02.10.2022

Source: results.cik.bg
Readme: ns2022/np/readme_02.10.2022.txt
...
ELECTIONS = [
    {
        "slug":    "ns2022",
        "name":    "Народно събрание 02.10.2022",
        "type":    "parliament",
        "date":    "2022-10-02",
        "round":   1,
        "rel_path": "ns2022/np",
...
def _extract_machine_data(p: list[str]) -> tuple[str, int | None, int | None] | None:
    ...
    readme_02.10.2022.txt, Формуляр № 32 (машинно гласуване):
      p[0]  = 1) № формуляр (32)
...
def parse_protocols(filepath: str, election_id: int, cur: sqlite3.Cursor) -> int:
    ...
    readme_02.10.2022.txt, Формуляр № 24 (paper-only domestic, Х):
      p[0]  = 1) № формуляр (24)
      p[1]  = 2) Пълен код на секция
      p[2]  = 3) Код на РИК
...
def parse_votes(filepath: str, election_id: int, cur: sqlite3.Cursor) -> int:
    ...
    readme_02.10.2022.txt — votes.txt:
      p[0] = 1) № формуляр
      p[1] = 2) Пълен код на секция
...
def parse(db_path: str | None = None) -> dict:
    conn, db_path = create_temp_db("ns2022", db_path)
    cur = conn.cursor()
    slugs = []
